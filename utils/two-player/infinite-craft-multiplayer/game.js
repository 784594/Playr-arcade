(function () {
	...
	const BASE_RECIPES = {
		...
		'steam::fire': { name: 'Engine', emoji: '🚗', tags: ['industry', 'heat'] },
		'steam::steam': { name: 'Engine', emoji: '🚗', tags: ['industry', 'motion'] },
		'engine::engine': { name: 'Rocket', emoji: '🚀', tags: ['industry', 'motion'] },
	...
	const els = {
		...
		craftLayout: document.querySelector('.craft-layout'),
	...
	function setRoomPanelMode(mode) {
		const nextMode = mode === 'join' ? 'join' : 'default';
		state.roomPanelMode = nextMode;
		if (els.roomDefaultFlow) {
			els.roomDefaultFlow.hidden = nextMode !== 'default';
		}
		if (els.joinRoomFlow) {
			els.joinRoomFlow.hidden = nextMode !== 'join';
		}
		if (nextMode === 'join' && els.joinRoomInput) {
	...
	function renderRoomControls() {
		...
		els.roomCodeDisplay.textContent = `Code: ${state.roomId} • Players: ${count}/${state.ownerRoomLimit}`;
	...
	function renderWorkspace() {
		...
		els.workspace.innerHTML = state.workspaceNodes.map((node) => {
	...
	async function createRoom() {
		...
		try {
			...
			showNotice(`Create room failed: ${error?.message || 'unknown error'}`);
		} finally {
	...
	async function joinRoom(roomIdInput, options = {}) {
		...
		try {
			...
			showNotice(`Room join failed: ${error?.message || 'unknown error'}`);
			return false;
		} finally {
	...
	function bindEvents() {
		...
		if (els.joinFlowBtn) {
			els.joinFlowBtn.addEventListener('click', () => {
				setRoomPanelMode('join');
			});
		}

		if (els.joinFlowBackBtn) {
			els.joinFlowBackBtn.addEventListener('click', () => {
				setRoomPanelMode('default');
			});
		}
